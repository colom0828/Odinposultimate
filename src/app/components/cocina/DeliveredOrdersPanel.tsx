...
import { KitchenOrder, ORDER_TYPE_CONFIG } from '../../types/cocina.types';
...
export function DeliveredOrdersPanel({ 
  ...
  const getDeliveryTime = (order: KitchenOrder) => {
    if (!order.horaLista) return 'N/A';
    const time = new Date(order.horaLista);
  ...
  return (
    <div className="mt-12">
      ...
      <div className="mb-6">
        <div 
          className="flex items-center justify-between p-6 bg-gradient-to-r from-slate-800 to-slate-900 dark:from-slate-900 dark:to-slate-950 rounded-2xl border border-slate-700 dark:border-slate-800 cursor-pointer hover:shadow-lg transition-all duration-300"
          onClick={() => setIsExpanded(!isExpanded)}
        >
          <div className="flex items-center space-x-4">
            <div className="w-12 h-12 bg-slate-700 dark:bg-slate-800 rounded-xl flex items-center justify-center">
              <LucideIcons.Archive className="w-6 h-6 text-slate-300" />
            </div>
            <div>
              ...
              <p className="text-sm text-slate-400">
                {orders.length} {orders.length === 1 ? 'orden completada' : 'órdenes completadas'}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center space-x-3">
            ...
            >
              <LucideIcons.ChevronDown className="w-6 h-6 text-slate-400" />
            </motion.div>
          </div>
        </div>
      </div>
                    ...
                    >
                      ...
                      <div className="flex items-start justify-between mb-3">
                        ...
                        <Button
                          ...
                          size="sm"
                          className="bg-red-100 hover:bg-red-200 dark:bg-red-900/20 dark:hover:bg-red-900/30 text-red-600 dark:text-red-400"
                        >
      ...
      {/* Dialog de confirmación - Eliminar todas */}
      <AlertDialog open={showDeleteAllDialog} onOpenChange={setShowDeleteAllDialog}>
        <AlertDialogContent className="bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-slate-900 dark:text-white">
              ¿Eliminar todas las órdenes entregadas?
            </AlertDialogTitle>
            <AlertDialogDescription className="text-slate-600 dark:text-slate-400">
              Esta acción eliminará permanentemente {orders.length} {orders.length === 1 ? 'orden' : 'órdenes'} del historial.
